...
"""
train_text_embedding_multitask_classifier.py

Train multitask classifiers using patient-level text embeddings generated from
large language models (LLMs) and language models (LMs) for prediction of:
- Sudden Cardiac Death (SCD)
- Pump Failure Death (PFD)
...
- Weighted BCE loss for class imbalance handling
- Reproducible training with deterministic CUDA settings
- Automatic loading of precomputed patient embeddings
...
- Fold CSV containing:
    ...
    - label
    - fold
- Embedding directory organized as:
    <embedding_dir>/<llm_model_name>/<lm_model_name>/<pid>_<llm>_<lm>.npy
...
- Per-fold best epoch and AUC scores
"""
