...
class AttentionMIL(nn.Module):
    ...
    Input:  T_seq (1, H, N)
    Output: z     (H,)
    Also returns attention weights a (N,) for interpretability.
    """
    ...
    def forward(self, T_seq: torch.Tensor):
        # T_seq: (1, H, N)
        X = T_seq.squeeze(0).transpose(0, 1)      # (N, H)
        scores = self.attn(X).squeeze(1)          # (N,)
        a = torch.softmax(scores, dim=0)          # (N,)
        z = torch.sum(X * a.unsqueeze(1), dim=0)  # (H,)
        return z, a
...
def _tune_threshold_for_f1(y_true: np.ndarray, y_prob: np.ndarray,
                          t_min: float = 0.05, t_max: float = 0.95, n: int = 181):
    """
    Tune threshold on validation probabilities to maximize F1.
    Returns (best_t, best_f1).
    """
...
def evaluate_metrics(model, loader, device, tune_threshold: bool = True):
    model.eval()
    y_true, y_prob = [], []
    ...
    # Decision threshold
    if tune_threshold:
        best_t, best_f1 = _tune_threshold_for_f1(y_true, y_prob)
    else:
    ...
    return acc, prec, rec, f1, auc, best_t, best_f1
...
def main():
    ...
    parser.add_argument("--no_threshold_tuning", action="store_true",
                        help="Disable validation threshold tuning (uses 0.5).")
    ...
    # -------- Class-weighted loss (computed from training fold) --------
    train_labels = train_df["label"].map(LABEL_MAP).values.astype(int)
    counts = np.bincount(train_labels, minlength=2)
    # Avoid divide-by-zero
    counts = np.maximum(counts, 1)
    weights = counts.sum() / counts  # standard inverse-frequency weights
    class_weights = torch.tensor(weights, dtype=torch.float32).to(device)
    logging.info(f"Class counts (train): {counts.tolist()} | CE weights: {weights.tolist()}")
    ...
    for epoch in range(args.epochs):
        ...
        acc, prec, rec, f1, auc, best_t, best_f1 = evaluate_metrics(
            model, val_loader, device, tune_threshold=(not args.no_threshold_tuning)
        )

        logging.info(
            f"Epoch {epoch+1} | Acc={acc:.3f} | Prec={prec:.3f} | Rec={rec:.3f} | "
            f"F1={f1:.3f} | AUC={auc:.3f} | "
            f"TunedThr={best_t:.3f} | TunedF1={best_f1:.3f}"
        )
        ...
        if auc > best_auc:
            ...
            torch.save(
                model.state_dict(),
                output_dir / "mil_tcn_attn_mil_best_auc.pt"
            )
