...
def preprocess_record(record_number_raw, base_path, output_path, skip_seconds):
    ...
    signal_out = cleaned.astype(np.float32)
    ...
    try:
        ...
        np.savez_compressed(
            save_path,
            signal=signal_out,
            rpeaks=rpeaks,          # KEY ADDITION
            fs=fs,
            record_name=record_id,
            n_samples=len(signal_out),
        )

        meta["save_time_s"] = round(time.time() - t4, 3)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Preprocess ECG with single-pass R-peak detection.")
