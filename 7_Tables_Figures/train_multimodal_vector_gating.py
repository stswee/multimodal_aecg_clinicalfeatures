...
def setup_logging(out_dir: Path):
    ...
    log_path = log_dir / f"train_multimodal_vector_gating_{ts}.log"
...
def gate_stats(g: torch.Tensor) -> dict:
    """
    g: (B, D) in [0,1]
    Returns summary stats that are easy to log + save.
    """
    g = g.detach()
    return {
...
def main():
    ...
    for epoch in range(args.epochs):
        ...
        z_scd, z_pfd, _, g_train = model(X_ecg_train_t, X_text_train_t)
        ...
        logging.info(
            ...
            f"Gate g: mean={gs['g_mean']:.4f} std={gs['g_std']:.4f} "
            f"min={gs['g_min']:.4f} max={gs['g_max']:.4f}"
        )
    ...
    if best_model_state is not None:
        ...
        raise RuntimeError("Best model was never set.")
    ...
    torch.save(
        {
            ...
            "best_gate_stats": best_gate_stats,
        },
        out / "best_model.pt",
    )

    # -----------------------------------------------------
    # Save validation predictions (best epoch)
    # -----------------------------------------------------
    if best_p_scd is not None and best_p_pfd is not None:
        np.savez_compressed(
            out / "val_predictions.npz",
            y_true_scd=y_val[:, 0].astype(np.int64),
            y_prob_scd=best_p_scd.astype(np.float32),
            y_true_pfd=y_val[:, 1].astype(np.int64),
            y_prob_pfd=best_p_pfd.astype(np.float32),
        )
        logging.info("Saved best-epoch validation predictions.")
    else:
        raise RuntimeError("Best predictions were never stored.")

    # -----------------------------------------------------
    # Recompute metrics from best predictions
    # -----------------------------------------------------
    acc_s, prec_s, rec_s, f1_s, auc_s = eval_head_np(y_val[:, 0], best_p_scd)
    acc_p, prec_p, rec_p, f1_p, auc_p = eval_head_np(y_val[:, 1], best_p_pfd)
    ...
    fold_summary = {
        ...
        # Gate stats
        "best_gate_mean": best_gate_stats["g_mean"],
        "best_gate_std": best_gate_stats["g_std"],
        "best_gate_min": best_gate_stats["g_min"],
        "best_gate_max": best_gate_stats["g_max"],
    }
    ...
    logging.info(
        f"Best gate g  : mean={best_gate_stats['g_mean']:.4f} std={best_gate_stats['g_std']:.4f} "
        f"min={best_gate_stats['g_min']:.4f} max={best_gate_stats['g_max']:.4f}"
    )
    logging.info("========================================")
