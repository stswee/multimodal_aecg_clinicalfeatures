...
def setup_logging(out_dir: Path):
    ...
    log_path = log_dir / f"train_multimodal_weighted_sum_{ts}.log"
...
def main():
    ...
    for epoch in range(args.epochs):
        ...
        z_scd, z_pfd, _, a_val = model(X_ecg_train_t, X_text_train_t)
        ...
        model.eval()
        with torch.no_grad():
            z_scd, z_pfd, _, a_val = model(X_ecg_val_t, X_text_val_t)
        ...
        a_scalar = float(a_val.item())
        b_scalar = 1.0 - a_scalar

        logging.info(
            f"Epoch {epoch+1} | "
            f"AUC={mean_auc:.3f} | "
            f"a(ECG)={a_scalar:.3f} | "
            f"b(Text)={b_scalar:.3f}"
        )
    ...
    torch.save(
        {
            ...
            "best_a": float(best_a) if best_a is not None else None,
            "best_b": float(best_b) if best_b is not None else None,
    ...
    # -----------------------------------------------------
    if best_p_scd is None or best_p_pfd is None:
        raise RuntimeError("Best validation probabilities were never stored.")
    ...
    logging.info(f"Saved fold outputs to {out}")
    ...
    logging.info("========================================")
    logging.info(f"Best mean AUC : {mean_auc:.4f}")
    logging.info(f"Best a (ECG)  : {best_a:.4f}")
    logging.info(f"Best b (Text) : {best_b:.4f}")
    logging.info("========================================")
