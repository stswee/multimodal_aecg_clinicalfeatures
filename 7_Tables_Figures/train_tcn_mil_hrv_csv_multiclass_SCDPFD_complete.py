...
- Saves best checkpoint by mean AUC
- Saves raw validation predictions per fold for external ROC construction
"""
...
def extract_and_save_embeddings_npz(
    ...
    for Xs, labels, batch_pids in tqdm(loader, desc=f"Saving {out_path.name}", ncols=100):
        X = Xs[0].to(device)
        pid = batch_pids[0]

        if X.numel() == 0 or X.size(0) < min_segments:
            continue

        _, _, z_shared, _ = model(X)
        z_np = z_shared.detach().cpu().numpy().astype(np.float32)

        pids.append(pid)
        embeddings.append(z_np)
        y_scd_all.append(int(labels[0, 0].item()))
        y_pfd_all.append(int(labels[0, 1].item()))

    if len(embeddings) == 0:
        logging.warning(f"No embeddings extracted for {out_path}")
        return
    ...
    np.savez_compressed(
        ...
        pids=np.array(pids),
        z=embeddings,
        y_scd=np.array(y_scd_all, dtype=np.int64),
        y_pfd=np.array(y_pfd_all, dtype=np.int64),
    )
...
def main():
    ...
    p.add_argument("--ecg_embedding_dir", type=Path, default=None, 
                   help="If set, save shared ECG embeddings per fold into this directory.")
    ...
    for e in range(args.epochs):
        ...
        if mean_auc > best_mean_auc:
            ...
            val_pred_path = out / "val_predictions.npz"
            np.savez_compressed(
                val_pred_path,
                y_true_scd=np.array(yts, dtype=np.int64),
                y_prob_scd=np.array(yps, dtype=np.float32),
                y_true_pfd=np.array(ytp, dtype=np.int64),
                y_prob_pfd=np.array(ypp, dtype=np.float32),
            )
            ckpt = {
    ...
    # =========================================================
    # Save Shared ECG Embeddings (Reusable for Multimodal)
    # =========================================================
    if args.ecg_embedding_dir is not None:
    
        emb_root = args.ecg_embedding_dir
        fold_dir = emb_root / f"val_fold_{args.val_fold}"
        fold_dir.mkdir(parents=True, exist_ok=True)
    
        best_ckpt_path = out / "best_model.pt"
        if best_ckpt_path.exists():
            ckpt = torch.load(best_ckpt_path, map_location=device)
            model.load_state_dict(ckpt["model_state_dict"])
            logging.info("Loaded BEST model for embedding extraction.")
        else:
            logging.warning("Best model checkpoint not found. Using final model state.")
    
        # Fresh loaders without shuffle
        ...
        logging.info(f"Saved ECG embeddings to {fold_dir}")
