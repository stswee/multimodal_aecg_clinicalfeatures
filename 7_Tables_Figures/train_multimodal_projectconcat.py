...
def setup_logging(out_dir: Path):
    ...
    log_path = log_dir / f"train_multimodal_projected_concat_{ts}.log"
...
class ProjectedConcatMultiHead(nn.Module):
    ...
    def forward(self, ecg, text):
        ...
        z_fused = torch.cat([h_ecg, h_text], dim=1)
...
def main():
    ...
    for epoch in range(args.epochs):
        ...
        z_scd, z_pfd, _ = model(X_ecg_train_t, X_text_train_t)
        ...
        if mean_auc > best_mean_auc:
            ...
            # Save best validation probabilities
    ...
    best_mean_auc = float(np.nanmean([auc_s, auc_p]))
