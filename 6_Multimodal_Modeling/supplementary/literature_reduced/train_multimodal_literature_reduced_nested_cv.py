...
TEXT_CONDITIONS: dict[str, tuple[str, str]] = {}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Nested-CV ECG plus literature-reduced clinical fusion sensitivity analysis."
    )
    parser.add_argument(
        "--stage",
        required=True,
    ...
    parser.add_argument(
        ...
        default="ADD_COMPLETE_CITATION_OR_DOI_BEFORE_MANUSCRIPT_USE",
    )
...
def load_text_matrix(
    ...
    artifact = np.load(path, allow_pickle=False)
    required = {"patient_ids", "embeddings", "text_sha256"}
    missing = required - set(artifact.files)
    if missing:
        raise ValueError(f"{path} lacks arrays: {sorted(missing)}")
...
def derive_literature_features(source: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    arb = pd.to_numeric(
        result["Angiotensin-II receptor blocker (yes=1)"], errors="coerce"
    )
    result[LITERATURE_NYHA_III] = np.where(nyha.isna(), np.nan, nyha.eq(3).astype(float))
    result[LITERATURE_ISCHEMIC] = np.where(
        etiology.isna(), np.nan, etiology.eq(2).astype(float)
    )
    result[LITERATURE_ACE_OR_ARB] = np.where(
        ace.eq(1) | arb.eq(1),
        1.0,
        np.where(ace.eq(0) & arb.eq(0), 0.0, np.nan),
    )
    definitions = {
        LITERATURE_NYHA_III: "1 if NYHA class == 3; 0 otherwise; missing if source missing",
        LITERATURE_ISCHEMIC: "1 if HF etiology - Diagnosis == 2 (ischemic); 0 otherwise; missing if source missing",
        LITERATURE_ACE_OR_ARB: "1 if ACE inhibitor or ARB == 1; 0 only if both == 0; otherwise missing",
    }
    audit = pd.DataFrame(
        [
            {
                ...
                "positive": int(pd.to_numeric(result[feature], errors="coerce").eq(1).sum()),
                "missing": int(pd.to_numeric(result[feature], errors="coerce").isna().sum()),
            }
...
def read_tabular(args: argparse.Namespace, folds: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    source = pd.read_csv(
        args.tabular_csv,
        sep=";",
        decimal=",",
    ...
    required = LITERATURE_RAW_DEPENDENCIES
    ...
    source, derivation_audit = derive_literature_features(source)
...
def build_tabular_preprocessor() -> ColumnTransformer:
    ...
    return ColumnTransformer(
        [
            ("continuous", continuous, LITERATURE_CONTINUOUS_FEATURES),
            ("categorical", categorical, LITERATURE_CATEGORICAL_FEATURES),
        ],
        remainder="drop",
        sparse_threshold=0,
    )
...
def stage_prepare(args: argparse.Namespace) -> None:
    ...
    tabular, derivation_audit = read_tabular(args, folds)
    audit_rows = []
    ...
    atomic_csv(setup / "literature_reduced_feature_derivation_audit.csv", derivation_audit)
    ...
    atomic_json(
        setup / "analysis_manifest.json",
        {
            ...
            "literature_reduced_continuous_features": LITERATURE_CONTINUOUS_FEATURES,
            "literature_reduced_categorical_features": LITERATURE_CATEGORICAL_FEATURES,
...
def stage_tune(args: argparse.Namespace) -> None:
    ...
    folds = read_folds(args)
    tabular = read_tabular(args, folds)[0] if pair == "ecg_literature_reduced_tabular" else None
