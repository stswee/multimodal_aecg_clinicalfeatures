#!/usr/bin/env python3
"""Post-analysis for completed detailed-response MUSIC multimodal predictions.
...
* paired bootstrap comparisons among the three selected-fusion modality pairs;
...
TASKS = ("scd", "pfd")
EXPECTED_PAIRS = ("ecg_full_text", "ecg_deterministic_text", "ecg_tabular")
EXPECTED_ARMS = (
    "concatenation",
...
def paired_selected_comparisons(frame: pd.DataFrame, replicates: int, seed: int) -> pd.DataFrame:
    ...
    comparisons = (
        ("ecg_tabular", "ecg_full_text"),
        ("ecg_deterministic_text", "ecg_full_text"),
        ("ecg_tabular", "ecg_deterministic_text"),
    )
    rows = []
    for task in TASKS:
...
def main() -> None:
    ...
    atomic_json(output / "posthoc_evaluation_manifest.json", {
        ...
        "text_embedding_policy": analysis_manifest["expected_text_embedding_policy"],
        ...
        "paired_comparison_scope": "three selected_fusion modality pairs",
