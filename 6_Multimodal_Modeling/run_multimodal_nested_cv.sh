#!/usr/bin/env bash
# ==============================================================================
# MUSIC FOUR-YEAR MULTIMODAL FUSION: NESTED CROSS-VALIDATION
# ==============================================================================
#
# Runs three modality pairs independently for SCD and PFD:
#   1. ECG + full LLM text without ECG impressions;
#   2. ECG + deterministic-template text without ECG impressions; and
#   3. ECG + prompt-matched structured variables without ECG impressions.
#
...
# cohort. Detailed LLM responses are represented with mean-pooled overlapping
# 512-token chunks; the preflight rejects any silently truncated text artifact.
# The competing endpoint is excluded from each binary task. The launcher
# distributes task-by-pair-by-outer-fold tuning and final fitting tasks across
# eight GPUs. A CPU orchestrator runs prepare, selection, aggregation, and
# bootstrap evaluation stages.
# ==============================================================================

set -euo pipefail

SESSION_NAME="multimodal_nested_4year_v4_detailed"
CONDA_ENV="shdb-af-analysis"

SCRIPT_DIR="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)"
SCRIPT_PATH="${SCRIPT_DIR}/$(basename -- "${BASH_SOURCE[0]}")"
PYTHON_SCRIPT="${SCRIPT_DIR}/train_multimodal_nested_cv.py"
...
OUTPUT_ROOT="${MUSIC_DIR}/multimodal_nested_4year_v4_detailed"
...
COMMON_ARGS=(
    ...
    --expected_text_pooling "${EXPECTED_TEXT_POOLING}"
    --expected_text_max_length "${EXPECTED_TEXT_MAX_LENGTH}"
    --expected_text_long_strategy "${EXPECTED_TEXT_LONG_STRATEGY}"
)
...
check_inputs() {
    ...
    [[ -s "${TEXT_RESULTS_ROOT}/combined_evaluation/combined_evaluation_manifest.json" ]] || {
        echo "Missing completed text-analysis manifest under ${TEXT_RESULTS_ROOT}" >&2
        exit 1
    }
