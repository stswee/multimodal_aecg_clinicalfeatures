...
- Loads best checkpoint from training (mil_model_best_auc.pt)
- Runs inference on validation fold
- Saves per-patient predictions
- Optionally saves attention weights
- Computes binary metrics (Acc, Prec, Rec, F1, ROC-AUC)

Example:
python infer_mil_ecg_attention_binary.py   --val_fold 0   --segments_dir ../../../../local3/sswee/music_download/physionet.org/files/music-sudden-cardiac-death/1.0.1/preprocessed_segments   --csv_path ../../../../local3/sswee/music_download/physionet.org/files/music-sudden-cardiac-death/1.0.1/music_patient_folds_5cv.csv   --model_path ../../../../local3/sswee/music_download/physionet.org/files/music-sudden-cardiac-death/1.0.1/mil_outputs_binary/val_fold_0/mil_model_best_auc.pt   --output_dir ../../../../local3/sswee/music_download/physionet.org/files/music-sudden-cardiac-death/1.0.1/mil_outputs_binary/val_fold_0_inference   --save_attention
"""
...
def main():
    ...
    assert args.num_classes == 2, "Binary model expected (num_classes=2)"
    ...
    with torch.no_grad():
        for segments, labels, patient_ids in tqdm(loader, desc="Inference"):
            ...
            prob_path = probs[1]
            pred = int(prob_path >= 0.5)

            y_true.append(label)
            y_pred.append(pred)
            y_prob.append(prob_path)
