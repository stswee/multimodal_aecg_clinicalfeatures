...
# ----------------------------
# Optimization target
#   scd  = optimize SCD (PFD censored)
#   pfd  = optimize PFD (SCD censored)
#   mean = multitask (original behavior)
# ----------------------------
OPT_TARGET="pfd"

# tmux session name (reflects optimization target)
SESSION_NAME="mil_temporal_survival_fold4_emb256_${OPT_TARGET}"

# Script location
SCRIPT_DIR="$( cd "$( dirname "${BASH_SOURCE[0]}" )" && pwd )"
PYTHON_SCRIPT="${SCRIPT_DIR}/train_mil_ecg_survival_SCDPFD.py"
...
OUTPUT_DIR="../../../../local3/sswee/music_download/physionet.org/files/music-sudden-cardiac-death/1.0.1/mil_outputs_temporal_survival_emb256_PFD"
...
echo "Optimization target: ${OPT_TARGET}"
echo
echo "Command:"
echo "${CMD}"
echo
