...
# Training
# =========================================================
def train_epoch(model, loader, optimizer, criterion, device, epoch, total_epochs, opt_target):
    model.train()
    losses = []
    ...
    for segments_list, times, es, ep, _ in tqdm(
        ...
        if opt_target == "scd":
            loss = criterion(batch_risk_scd, times, es)
        elif opt_target == "pfd":
            loss = criterion(batch_risk_pfd, times, ep)
        else:
...
def main():
    ...
    type=str,
    default="mean",
    choices=["scd", "pfd", "mean"],
    help="Optimization target: scd (PFD censored), pfd (SCD censored), mean (both)")
    ...
    for epoch in tqdm(range(args.epochs), desc="Epochs", ncols=80):
        train_loss = train_epoch(model, train_loader, optimizer, criterion, device, epoch, args.epochs, args.opt_target)
        ...
        if args.opt_target == "scd":
            score = val_metrics["val_cindex_scd"]
            ckpt_name = "model_best_scd_cindex.pt"
        
        elif args.opt_target == "pfd":
            score = val_metrics["val_cindex_pfd"]
            ckpt_name = "model_best_pfd_cindex.pt"
        
        else:
            score = val_metrics["val_cindex_mean"]
            ckpt_name = "model_best_mean_cindex.pt"
        
        if np.isfinite(score) and score > best_score:
            best_score = score
            torch.save(model.state_dict(), fold_out / ckpt_name)
            logging.info(
                f"New BEST model saved at epoch {epoch+1} "
                f"({args.opt_target.upper()} C-index={score:.4f})"
            )


    logging.info(
        f"Training complete. Best {args.opt_target.upper()} validation C-index = {best_score:.4f}. "
        f"Metrics log: {metrics_csv}"
    )
