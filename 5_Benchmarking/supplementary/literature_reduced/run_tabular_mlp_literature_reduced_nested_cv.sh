#!/usr/bin/env bash
# Endpoint-specific architecture-matched reduced-clinical MLP sensitivity.
...
OUTPUT_ROOT="${MUSIC_DIR}/tabular_mlp_literature_reduced_continuous_lvef_4year_v1"
TABULAR_REFERENCE="${MUSIC_DIR}/tabular_literature_reduced_continuous_lvef_4year_v1/evaluation/models/literature_reduced/selected_tabular/pooled_predictions_calibrated_and_classified.csv"
SESSION_NAME="tabular_mlp_literature_reduced_continuous_lvef_4year_v1"
...
if [[ "${1:-}" == "_worker" ]]; then
  ...
  set +e
  CUDA_VISIBLE_DEVICES="${gpu}" python train_tabular_mlp_literature_reduced_nested_cv.py \
...
if [[ "${1:-}" == "_orchestrator" ]]; then
  ...
  extra=()
  [[ "${overwrite}" == "1" ]] && extra+=(--overwrite)
  python train_tabular_mlp_literature_reduced_nested_cv.py --stage aggregate \
    "${COMMON_ARGS[@]}" "${extra[@]}" 2>&1 | tee "${LOG_DIR}/aggregate.log"
  status="${PIPESTATUS[0]}"
  echo "${status}" > "${STATUS_DIR}/aggregate.status"
  echo "Aggregation exit status: ${status}"
  exit "${status}"
fi

if [[ "${1:-}" == "--aggregate-only" ]]; then
  for required in "${TABULAR_CSV}" "${FOLDS_CSV}" "${TABULAR_REFERENCE}" "${SCRIPT_DIR}/train_tabular_mlp_literature_reduced_nested_cv.py"; do
    [[ -f "${required}" ]] || { echo "Missing required file: ${required}" >&2; exit 1; }
  done
  mkdir -p "${LOG_DIR}"
  activate_environment
  python train_tabular_mlp_literature_reduced_nested_cv.py --stage aggregate \
    "${COMMON_ARGS[@]}" 2>&1 | tee "${LOG_DIR}/aggregate_recovery.log"
  status="${PIPESTATUS[0]}"
  echo "${status}" > "${STATUS_DIR}/aggregate.status"
  echo "Aggregate-only exit status: ${status}"
  exit "${status}"
fi
...
if [[ "${1:-}" == "--overwrite" ]]; then
  ...
  echo "Usage: $0 [--overwrite|--aggregate-only]" >&2
  exit 2
fi
...
rm -f "${STATUS_DIR}/aggregate.status"
activate_environment
prepare_extra=()
[[ "${overwrite}" -eq 1 ]] && prepare_extra+=(--overwrite)
python train_tabular_mlp_literature_reduced_nested_cv.py --stage prepare "${COMMON_ARGS[@]}" "${prepare_extra[@]}"

tmux new-session -d -s "${SESSION_NAME}" -n orchestrator \
  "bash '${SCRIPT_DIR}/run_tabular_mlp_literature_reduced_nested_cv.sh' _orchestrator '${overwrite}'; status=\$?; echo 'Orchestrator exit status:' \${status}; exec bash"
for fold in 0 1 2 3 4; do
  tmux new-window -t "${SESSION_NAME}" -n "fold${fold}" \
    "bash '${SCRIPT_DIR}/run_tabular_mlp_literature_reduced_nested_cv.sh' _worker '${fold}' '${fold}' '${overwrite}'; status=\$?; echo 'Worker exit status:' \${status}; exec bash"
done
