...
FEATURE_SETS = {
    "prompt_matched_no_ecg": {
        "continuous": PROMPT_CONTINUOUS_FEATURES,
        "categorical": PROMPT_CATEGORICAL_FEATURES,
        "description": "Structured variables supplied to the no-ECG LLM prompt.",
    },
    "prompt_matched_with_ecg": {
        "continuous": PROMPT_CONTINUOUS_FEATURES,
        "categorical": PROMPT_CATEGORICAL_FEATURES + ECG_IMPRESSION_FEATURES,
        "description": "Prompt-matched variables plus five Holter ECG impressions.",
    },
}
...
def prepare_analysis_data(args: argparse.Namespace) -> pd.DataFrame:
    ...
    missing_features = set(required_features) - set(source.columns)
    if missing_features:
    ...
    anticoagulant_audit = validate_anticoagulant_feature(dataframe, args)
...
def write_setup_outputs(
    ...
    sentinel_map: dict,
    anticoagulant_audit: dict[str, Any],
) -> None:
...
def fit_bundle(
    ...
    transformed = np.asarray(preprocessor.fit_transform(train[features]), dtype=np.float64)
...
def predict_bundle(bundle: dict, dataframe: pd.DataFrame) -> np.ndarray:
    transformed = np.asarray(
        bundle["preprocessor"].transform(dataframe[bundle["features"]]),
        dtype=np.float64,
    )
    return np.column_stack(
        [
            bundle["estimators"][task].predict_proba(transformed)[:, 1]
            for task in TASKS
        ]
    )
    ...
    for outer_fold in range(args.outer_splits):
        ...
        for selection_type, family in (
            ...
            candidates = (
                fold_results
                if family is None
                else fold_results[fold_results["family"].eq(family)]
            )
            selected = select_candidate(candidates, args.auc_tolerance)
            rows.append(
                {
                    "feature_set": feature_set,
                    "outer_fold": outer_fold,
                    ...
                    "inner_mean_roc_auc": selected["mean_roc_auc"],
                    "inner_mean_pr_auc": selected["mean_pr_auc"],
                    "configuration": selected["configuration"],
                    "selection_rule": (
                        f"within {args.auc_tolerance:g} of maximum pooled inner mean "
                        "ROC-AUC; then lowest prespecified complexity; then highest "
                        "pooled inner mean PR-AUC; then candidate name"
                    ),
                }
            )
...
def fit_final_models(
    ...
    for outer_fold in range(args.outer_splits):
        ...
        for arm in ("penalized_logistic", "gradient_boosting", "selected_tabular"):
            ...
            config = json.loads(selected["configuration"])
            destination = (
                args.output_root
                / "final_models"
                / feature_set
